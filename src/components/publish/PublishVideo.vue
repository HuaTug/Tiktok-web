  ...
  methods: {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          publishVideo(this.videoForm).then(res => {
            // Refactored-TikTok backend uses code 0 for success
            if (res.code === 0 || res.code === 200) {
              this.$message.success(res.message || res.msg || '发布成功')
            } else {
              this.$message.error(res.message || res.msg || '发布失败')
            }
          }).catch(err => {
    ...
    handleVideoSuccess(res, file) {
      ...
      if (res.code === 0 || res.code === 200) {
        ...
        this.loading = false
        this.videoForm.videoUrl = res.data?.videoUrl || res.data?.video_url || ''
        this.videoForm.coverImage = res.data?.vframe || res.data?.cover_image || ''
      } else {
    ...
    handleInputConfirm() {
      if (this.videoTags.length > 5) {
        ...
        }).catch(err => console.log('Save video tag failed'))
      }
    },
    // 展示标签输入框
    showTagInput() {
